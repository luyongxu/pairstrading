...
select_pairs <- function(train, coin_pairs, adf_threshold) { 
...
#' Description   
#' Performs rolling linear regression of log(coin y) on log(coin x) using a defined rolling window length over 
#' the test set.  
#' 
#' Arguments  
#' train: A dataframe generated by prepare_data() that represents the training set for the coin pair.  
...
backtest_pair <- function(train, test, coin_y, coin_x, rolling_window, stop_threshold, signal_logic) { 
...
plot_single <- function(train, test, coin_y, coin_x, rolling_window, stop_threshold, signal_logic) { 
  model <- train_model(train = train, 
                       test = test, 
                       coin_y = coin_y, 
                       coin_x = coin_x, 
