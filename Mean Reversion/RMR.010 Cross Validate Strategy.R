...
#' of the coin with itself are removed. The function returns a dataframe containing the coin pairs.  
#' 
#' Arguments  
...
create_pairs <- function(quote_currency) { 
  if (quote_currency == "USDT") coin_list <- c("USDT_BTC", "USDT_DASH", "USDT_ETH", "USDT_LTC", "USDT_REP", "USDT_XMR", "USDT_ZEC")
  if (quote_currency == "BTC") coin_list <- c("BTC_DASH", "BTC_ETH", "BTC_LTC", "BTC_REP", "BTC_XEM", "BTC_XMR", "BTC_ZEC")
...
#' Description   
#' A rolling linear regression of log(coin y) on log(coin x) using a defined window length. Returns statistics 
#' of interest calculated from a rolling regression over the test set.  
#' 
#' Arguments  
#' train: A dataframe generated by prepare_data() that represents the training set for the coin pair.  
...
#' window_length: The number of observations used in each iteration of a rolling linear regression.  
train_model <- function(train, test, coin_y, coin_x, window_length) { 
  # Calculate spread in training set 
  model <- lm(log(train[[coin_y]]) ~ log(train[[coin_x]])) 
  train <- train %>% 
    mutate(spread = model[["residuals"]]) 
  # Perform rolling linear regression 
  combined <- bind_rows(train, test)  
  intercept <- rep(NA, nrow(combined)) 
  hedge_ratio <- rep(NA, nrow(combined)) 
  for (i in (window_length+1):nrow(combined)) { 
    model <- lm(log(combined[[coin_y]][(i-window_length):i]) ~ log(combined[[coin_x]][(i-window_length):i])) 
    intercept[i] <- coef(model)[1] 
    hedge_ratio[i] <- coef(model)[2]
  } 
  intercept <- intercept[(nrow(train)+1):nrow(combined)] 
  hedge_ratio <- hedge_ratio[(nrow(train)+1):nrow(combined)] 
  # Calculate spread in test set
  test <- test %>% 
    mutate(spread = log(test[[coin_y]]) - log(test[[coin_x]]) * hedge_ratio - intercept) 
  # Combine train and test to calculate rolling z-score
  result <- bind_rows(train %>% mutate(source = "train"), 
                      test %>% mutate(source = "test")) %>% 
    mutate(spread_z = (spread - roll_mean(spread, n = window_length, fill = NA, align = "right")) / 
             roll_sd(spread, n = window_length, fill = NA, align = "right")) %>% 
    filter(source == "test")
...
generate_signals <- function(train, test, coin_y, coin_x, threshold_z) { 
  model <- train_model(train, test, coin_y, coin_x, window_length)    
  df_signals <- test %>% 
    mutate(spread = model[["spread"]], 
           ...
           signal_long = ifelse(lag_spread_z <= -3.0 & lag_spread_z > -4.0, 1.00, signal_long), 
           signal_long = ifelse(lag_spread_z <= -4.0 & lag_spread_z > -5.0, 1.00, signal_long), 
           signal_long = ifelse(lag_spread_z <= -5.0 & lag_spread_z > -6.0, 1.00, signal_long), 
           signal_long = ifelse(lag_spread_z <= -6.0 & lag_spread_z > -7.0, 1.00, signal_long), 
           ...
           signal_short = ifelse(lag_spread_z >= 3.0 & lag_spread_z < 4.0, -1.00, signal_short), 
           signal_short = ifelse(lag_spread_z >= 4.0 & lag_spread_z < 5.0, -1.00, signal_short), 
           signal_short = ifelse(lag_spread_z >= 5.0 & lag_spread_z < 6.0, -1.00, signal_short), 
           signal_short = ifelse(lag_spread_z >= 6.0 & lag_spread_z < 7.0, -1.00, signal_short), 
...
plot_many <- function(pricing_data, time_resolution, cutoff_date, train_window, test_window, threshold_z) { 
  ...
  for (i in 1:min(3, nrow(selected_pairs))) { 
...
test_window <- days(16) 
window_length <- 86400 / time_resolution * as.numeric(train_window) / 86400 
test_by <- "16 days" 
threshold_z <- 4 
