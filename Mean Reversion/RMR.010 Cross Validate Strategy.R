...
#' # 13. Individual Pairs Cross Validation March 2017 
#' This section displays results from individual cointegrated coin pairs selected from a training set comprised of data  
#' from May 1, 2017 to July 1, 2017. The model is fit using the training set and evaluated on a test set comprised 
#' data from July 1, 2017 to July 31, 2017.  
...
#' # 15. Strategy Rolling Cross Validation Time Resolution 7200
...
for (test_date in test_dates) { 
  ...
  test <- test %>% 
    mutate(return_strategy_150 = backtest_strategy(train, test, select_pairs(train, create_pairs()), 1.50), 
           return_strategy_175 = backtest_strategy(train, test, select_pairs(train, create_pairs()), 1.75), 
           return_strategy_200 = backtest_strategy(train, test, select_pairs(train, create_pairs()), 2.00), 
           return_strategy_225 = backtest_strategy(train, test, select_pairs(train, create_pairs()), 2.25), 
           return_strategy_250 = backtest_strategy(train, test, select_pairs(train, create_pairs()), 2.50), 
           return_strategy_275 = backtest_strategy(train, test, select_pairs(train, create_pairs()), 2.75), 
           return_strategy_300 = backtest_strategy(train, test, select_pairs(train, create_pairs()), 3.00), 
           return_strategy_325 = backtest_strategy(train, test, select_pairs(train, create_pairs()), 3.25), 
           return_strategy_350 = backtest_strategy(train, test, select_pairs(train, create_pairs()), 3.50), 
           return_strategy_375 = backtest_strategy(train, test, select_pairs(train, create_pairs()), 3.75), 
           return_strategy_400 = backtest_strategy(train, test, select_pairs(train, create_pairs()), 4.00), 
           return_strategy_change_150 = return_strategy_150 / lag(return_strategy_150, 1) - 1, 
           return_strategy_change_175 = return_strategy_175 / lag(return_strategy_175, 1) - 1, 
           return_strategy_change_200 = return_strategy_200 / lag(return_strategy_200, 1) - 1, 
           return_strategy_change_225 = return_strategy_225 / lag(return_strategy_225, 1) - 1, 
           return_strategy_change_250 = return_strategy_250 / lag(return_strategy_250, 1) - 1, 
           return_strategy_change_275 = return_strategy_275 / lag(return_strategy_275, 1) - 1, 
           return_strategy_change_300 = return_strategy_300 / lag(return_strategy_300, 1) - 1, 
           return_strategy_change_325 = return_strategy_325 / lag(return_strategy_325, 1) - 1, 
           return_strategy_change_350 = return_strategy_350 / lag(return_strategy_350, 1) - 1, 
           return_strategy_change_375 = return_strategy_375 / lag(return_strategy_375, 1) - 1, 
           return_strategy_change_400 = return_strategy_400 / lag(return_strategy_400, 1) - 1) %>% 
    mutate_all(funs(ifelse(is.na(.), 0, .)))
  results <- bind_rows(results, test)
} 
results <- results %>% 
  mutate(return_strategy_cumulative_150 = cumprod(1 + return_strategy_change_150), 
         return_strategy_cumulative_175 = cumprod(1 + return_strategy_change_175), 
         return_strategy_cumulative_200 = cumprod(1 + return_strategy_change_200), 
         return_strategy_cumulative_225 = cumprod(1 + return_strategy_change_225), 
         return_strategy_cumulative_250 = cumprod(1 + return_strategy_change_250), 
         return_strategy_cumulative_275 = cumprod(1 + return_strategy_change_275), 
         return_strategy_cumulative_300 = cumprod(1 + return_strategy_change_300), 
         return_strategy_cumulative_325 = cumprod(1 + return_strategy_change_325), 
         return_strategy_cumulative_350 = cumprod(1 + return_strategy_change_350), 
         return_strategy_cumulative_375 = cumprod(1 + return_strategy_change_375), 
         return_strategy_cumulative_400 = cumprod(1 + return_strategy_change_400))
ggplot(results, aes(x = date_time)) + 
  geom_line(aes(y = return_strategy_cumulative_150, colour = "1.50"), size = 1) + 
  geom_line(aes(y = return_strategy_cumulative_175, colour = "1.75"), size = 1) + 
  geom_line(aes(y = return_strategy_cumulative_200, colour = "2.00"), size = 1) + 
  geom_line(aes(y = return_strategy_cumulative_225, colour = "2.25"), size = 1) + 
  geom_line(aes(y = return_strategy_cumulative_250, colour = "2.50"), size = 1) + 
  geom_line(aes(y = return_strategy_cumulative_275, colour = "2.75"), size = 1) + 
  geom_line(aes(y = return_strategy_cumulative_300, colour = "3.00"), size = 1) + 
  geom_line(aes(y = return_strategy_cumulative_325, colour = "3.25"), size = 1) + 
  geom_line(aes(y = return_strategy_cumulative_350, colour = "3.50"), size = 1) + 
  geom_line(aes(y = return_strategy_cumulative_375, colour = "3.75"), size = 1) + 
  geom_line(aes(y = return_strategy_cumulative_400, colour = "4.00"), size = 1) + 
  geom_hline(yintercept = 1, colour = "black") + 
  labs(title = "Strategy Return vs Buy Hold Return", x = "Date", y = "Cumulative Return") 

#' # 16. Strategy Rolling Cross Validation Time Resolution 14400
#' This section displays results fro ma pairs trading strategy using an equally-weighted portfolio of cointegrated 
#' coins where new coins are selected each month. The cross validation is run iteratively through time using a test 
#' set comprised of data from January 1, 2017 to October 1, 2017. 
test_dates <- (c("2017-01-01", "2017-02-01", "2017-03-01", "2017-04-01", "2017-05-01", "2017-06-01", 
...
for (test_date in test_dates) { 
  ...
  print(str_c("Cross validating strategy. using train set from ", test_date - months(2) , " to ", test_date, 
              " and test set from ", test_date, " to ", test_date + months(1), "."))  
  train <- prepare_data(df = pricing_data, time_resolution = 14400, start_date = test_date - months(2), end_date = test_date)
  test <- prepare_data(df = pricing_data, time_resolution = 14400, start_date = test_date, end_date = test_date + months(1))
  test <- test %>% 
    mutate(return_strategy_150 = backtest_strategy(train, test, select_pairs(train, create_pairs()), 1.50), 
           return_strategy_175 = backtest_strategy(train, test, select_pairs(train, create_pairs()), 1.75), 
           return_strategy_200 = backtest_strategy(train, test, select_pairs(train, create_pairs()), 2.00), 
