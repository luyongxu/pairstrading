...
train_model <- function(train, test, coin_y, coin_x, window_length) { 
  ...
  combined <- bind_rows(train, test) %>%  
    mutate(y = log(.[[coin_y]]), x = log(.[[coin_x]])) %>% 
    select(y, x)
  rolling_coef <- rollapply(data = combined, 
                            width = window_length, 
                            FUN = function(df) { 
                              df <- as_tibble(df)
  ...
  result <- bind_rows(train %>% mutate(source = "train"), 
    ...
    mutate(rolling_mean = roll_mean(spread, n = window_length, fill = NA, align = "right"), 
           rolling_sd = roll_sd(spread, n = window_length, fill = NA, align = "right"), 
...
#' coin_y: A string indicating the dependent coin in the coin pair regression.  
#' coin_x: A string indicating the independent coin in the coin pair regression.  
backtest_pair <- function(train, test, coin_y, coin_x) { 
...
backtest_strategy <- function(train, test, selected_pairs) { 
...
#' coin_y: A string indicating the dependent coin in the coin pair regression.  
#' coin_x: A string indicating the independent coin in the coin pair regression.  
plot_single <- function(train, test, coin_y, coin_x) { 
  model <- train_model(train, test, coin_y, coin_x, window_length)    
  df_plot <- test %>% 
    mutate(spread = model[["spread"]], 
           spread_z = model[["spread_z"]], 
...
test_window <- days(16) 
window_length <- 86400 / time_resolution * as.numeric(days(16)) / 86400 
test_by <- "16 days" 
