...
#' # 2. Train Model Rolling 
#' Description  
#' Trains a model using a rolling regression using either ols, tls, or a non-parametric regression method.  
#' 
#' Arguments  
#' See the train_model() function.   
#' 
#' Value  
#' Returns a list containing the intercept, hedge ratio, spread, and spread z-score over the test set.  
train_model_rolling <- function(train, test, coin_y, coin_x, params) { 
  ...
  # Calculate rolling coefficients if regression type is OLS 
  ...
  # Calculate rolling coefficients if regression type is TLS 
  ...
  # Combine train and test to calculate rolling spread z-score for the test set  
  ...
  # prices. For backtesting position calculation purposes, set the intercept to 0 and hedge ratio to 1.  
...
#' # 3. Train Model Fixed 
#' Description  
#' Trains a model using a fixed regression using either ols, tls, or a non-parametric regression method.  
#' 
#' Arguments  
#' See the train_model() function.  
#' 
#' Value  
#' Returns a list containing the intercept, hedge ratio, spread, and spread z-score over the test set.  
train_model_fixed <- function(train, test, coin_y, coin_x, params) { 
  ...
  # If regression is non-parametric 
  if (params[["regression_type"]] == "non-parametric") { 
    train <- train %>% 
...
train_model <- function(train, test, coin_y, coin_x, params) { 
  ...
  # Use either a rolling or fixed regression 
  if (params[["spread_type"]] == "rolling") { 
    model <- train_model_rolling(train = train, 
                                 test = test, 
                                 coin_y = coin_y, 
                                 coin_x = coin_x, 
                                 params = params)
  } 
  # If calculation of spread uses a fixed regression
  if (params[["spread_type"]] == "fixed") { 
    model <- train_model_fixed(train = train, 
                               test = test, 
                               coin_y = coin_y, 
                               coin_x = coin_x, 
                               params = params)
